    ...
    return (
      <div
        ...
        className={cn(
          ...
          direction === "vertical" && "flex flex-col items-start gap-2",
          className,
        )}
        {...args}>
        ...
        <h3
          className={cn(
            'feature-card-title text-xl font-medium',
            direction === 'vertical' && 'mt-4',
          )}>
          {title}
        </h3>
        {description && (
          <p className={cn('feature-card-description text-sm text-neutral-500 leading-6')}>{description}</p>
        )}
      </div>
    );
  },
